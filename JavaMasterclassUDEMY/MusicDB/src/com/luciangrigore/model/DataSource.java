package com.luciangrigore.model;
...
public class DataSource {
    ...
    public static final String CONNECTION_STRING = "jdbc:sqlite:C:\\Users\\LuciGrigore\\Documents\\UDEMYCompleteJavaMasterclass\\MusicDB\\" + DB_NAME;
    ...
    public List<SongArtist> queryArtistForSong(String songName, int sortOrder) {
        ...
        try (Statement statement = conn.createStatement();
             ResultSet resultSet = statement.executeQuery(sb.toString())) {
            List<SongArtist> data = new ArrayList<>();

            while (resultSet.next()) {
                SongArtist p = new SongArtist();
                p.setAlbumName(resultSet.getString(3));
                p.setTrack(resultSet.getInt(2));
                p.setArtistName(resultSet.getString(1));
                data.add(p);
            }
