...
void printare_coada(struct queue *q) {
  ...
  // distanta de la nodul radacina al subarborelui generat de algoritm la nodul curent
  // culoarea nodului curent
  ...
  for (i = 0, t = 1; i < n; ++i) {
    if (vizitat_bfs[i] == 0) { // daca nu am vizitat nodul curent
      ...
      while (is_empty(q) == 0) { // cat timp coada nu este goala
        ...
        for (j = 0; j < n; ++j) {
          // daca exista muchie de la nodul din coada la cel curent in iterare din graf
          // si acesta nu a fost vizitat
...
void explorare(int timp, int u) {
  d_dfs[u] = ++timp; // atribui timpul de inceput al nodului curent
  ...
  for (j = 0; j < n; ++j) {
    // daca exista muchie de la nodul curent la acesta si cel din urma este inca "alb"
  ...
  f_dfs[u] = ++timp; // atribui timpul de finalizare al nodului curent
}

// functia principala care realizeaza parcurgerea in adancime
void DFS() {
  int t = 0; // timpul algoritmului
  int i; // iteratorul
  ...
  for (i = 0; i < n; ++i) {
    if (vizitat_dfs[i] == 0) { // daca nodul nu a fost vizitat inca
      printf("\n\t\t---\n");
      explorare(t, i); // apelez functia de explorare pentru acesta
