...
void destroyStrElement(void *elem) { free((char *)elem); }
...
void destroyPriceElement(void *price) { free(price); }
...
Range *modelPriceRangeQuery(char *fileName, TTree *tree, char *m1, char *m2,
                            long p1, long p2) {
  // initializarea elementului de tip Range cu o capacitate initiala
  // de 10 elemente
  Range *range = (Range *)malloc(sizeof(Range));
...
int main(void) {
  ...
  TTree *priceTree =
      createTree(createPriceElement, destroyPriceElement, createIndexInfo,
                 destroyIndexInfo, comparePrice);
  if (priceTree == NULL)
    goto NullPointerException;
  ...
  Range *range4 =
      modelPriceRangeQuery("input.csv", modelTree, "L2", "M", 300, 600);
  if (range4 == NULL)
    goto NullPointerException;
