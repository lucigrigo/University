...
test_homework() {
	if [[ "$1" = "1" || "$1" = "holograme" ]]; then
		test_holograme
	elif [[ "$1" = "2" || "$2" = "scurt" ]]; then
		test_scurt
	elif [[ "$1" = "3" || "$3" = "umbre" ]]; then
		test_umbre
	elif [[ "$1" = "cs" ]]; then
...
test_holograme() {
	n_tests=50
	src_names=(holograme.c holograme.cpp holograme.java)
	tests=( $(seq 0 $(($n_tests - 1))) )
	points=( $(printf '1 %.0s' $(seq 0 $(($n_tests - 1)))) )
	pmax=50

	run_problem holograme 1
}

# Task 2
test_scurt() {
	n_tests=25
	src_names=(scurt.c scurt.cpp scurt.java)
	tests=( $(seq 0 $(($n_tests - 1))) )
	points=( $(printf '2 %.0s' $(seq 0 $(($n_tests - 1)))) )
	pmax=50
...
test_umbre() {
	n_tests=25
	src_names=(umbre.c umbre.cpp umbre.java)
	tests=( $(seq 0 $(($n_tests - 1))) )
	points=( $(printf '2 %.0s' $(seq 0 $(($n_tests - 1)))) )
	pmax=50
