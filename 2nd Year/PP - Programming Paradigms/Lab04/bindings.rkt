...
(define (compute-length get-line-segment get-start-point get-end-point)
  (let* ((segment (get-line-segment)) (inceput (get-start-point segment)) (final (get-end-point segment)))
    (sqrt (+ (expt (- (car final) (car inceput)) 2)
          (expt (- (cdr final) (cdr inceput)) 2)))))
...
(define (distance p1 p2)
  (let ((fct (λ (seg) (compute-length (λ () seg) get-start-point get-end-point))))
    (fct (cons p1 p2))))
...
(define (compute-perimeter points)
  (let comp ((points (append points (list (car points)))) (p 0))
    (if (= (length points) 1)
        p
        (comp (cdr points) (+ p (distance (car points) (cadr points)))))))

;(define (compute-perimeter points)
;  (let comp2 ((points points) (p (distance (take-right points 1) (car points)))) ;; Aici take-right nu stiu de ce nu imi intoarce doar ultimul punct ca pereche, sau il returneaza ciudat.
;    (if (= (length points) 1)
;        p
;        (comp2 (cdr points) (+ p (distance (car points) (cadr points)))))))
...
(define (get-sum-helper elems sum)
  (if (null? elems)
      sum
      (get-sum-helper (cdr elems) (+ sum (car elems)))))

(define (get-sum elems)
  (get-sum-helper elems 0))

(define (3-sequence-max numbers separator)
  (let finder ((numbers numbers) (max-until-now 0))
    (if (null? numbers)
        max-until-now
        (let-values ([(current-list rest-of-the-list)
                      (splitf-at numbers (λ (elem) (not (equal? elem separator))))])
              (if (> (get-sum current-list) max-until-now)
                  (if (null? rest-of-the-list)
                      (get-sum current-list)
                      (finder (cdr rest-of-the-list) (get-sum current-list)))
                  (if (null? rest-of-the-list)
                      max-until-now
                      (finder (cdr rest-of-the-list) max-until-now)))))))
                  

; (splitf-at '(1 0 2 0 3) (λ (elem) (not (equal? elem 0))))
