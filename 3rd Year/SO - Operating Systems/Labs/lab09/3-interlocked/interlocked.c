...
DWORD WINAPI thread_function(LPVOID arg)
{
	...
	for (i = 0; i < NO_ROUNDS; i++) {
		...
		// problema: succesiunea a 2 op. atomice nu este atomica -> rez. diferite la rulare
	}
...
DWORD WINAPI thread_function_mutex(LPVOID arg)
{
	...
	for (i = 0; i < NO_ROUNDS; i++) {
		 ...
		 */
		dwRet = WaitForSingleObject(hMutex, 1000);
		DIE(dwRet == WAIT_FAILED, "mutex wait fail");
		...
		dwRet = ReleaseMutex(hMutex);
		DIE(dwRet == 0, "mutex release fail");
	}
