...
static void do_io_async(void)
{
	...
	ov = (OVERLAPPED *) calloc(n_files, sizeof(OVERLAPPED));
	DIE(ov == NULL, "overlapped calloc err");
	...
	for (i = 0; i < n_files; ++i) {
		...
		if (dwRet == FALSE) {
			dwRet = GetLastError();
			DIE(dwRet != ERROR_IO_PENDING, "write file err");
		}
	}
	...
	for (i = 0; i < n_files; ++i) {
		dwRet = GetOverlappedResult(
			...
			&bytes_written,
			TRUE);
		DIE(dwRet == FALSE, "get overlapped result err");
	}
}
