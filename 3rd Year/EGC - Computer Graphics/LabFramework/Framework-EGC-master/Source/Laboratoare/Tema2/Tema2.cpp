...
void Tema2::Init()
{
    ...
    // generating initial platforms
    {
        ...
        //     }
        // }
        last_z = SpawnNextPlatforms(last_z, initial_platform_positions);
    }
}
...
void Tema2::AnimatePlatforms(float deltaTimeSeconds)
{
    ...
    {
        last_z = SpawnNextPlatforms(last_z, platform_positions, platform_colors,
...
void Tema2::AnimatePlayer(float deltaTimeSeconds)
{
    ...
    if (is_jumping)
    {
        if (player_position.y >= .5f)
        {
            ...
            player_position.y += (player_jump_speed * elapsed_jump) / 4.5f - (player_jump_speed * ((float)(pow(elapsed_jump, 2)) / 2.2f));
        }
        else
        {
            is_jumping = false;
