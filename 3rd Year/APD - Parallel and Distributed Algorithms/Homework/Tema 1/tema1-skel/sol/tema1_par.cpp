...
void *f_thread(void *args)
{
    ...
    if (thread_id == 0)
    {
        ...
        int width = (julia_args->x_max - julia_args->x_min) / julia_args->resolution;
        int height = (julia_args->y_max - julia_args->y_min) / julia_args->resolution;
            ...
            for (int h = 0; h < julia_args->height; ++h)
            {
                       ...
                       step < julia_args->iterations)
                {
                    ...
                    z.a = pow(z_aux.a, 2) - pow(z_aux.b, 2) +
                          julia_args->c_julia.a;
                    z.b = 2 * z_aux.a * z_aux.b +
                          julia_args->c_julia.b;
    ...
    if (thread_id == 0)
    {
        write_output_file(fjulia_out, julia, julia_args->width,
                          julia_args->height);
    ...
    // Mandelbrot
    {
        ...
        for (int w = w_start; w < w_end; ++w)
        {
            ...
            {
                TComplex c = {.a = w * mandelbrot_args->resolution +
                                   mandelbrot_args->x_min,
                              .b = h * mandelbrot_args->resolution +
                                   mandelbrot_args->y_min};
                TComplex z = {.a = 0, .b = 0};
                int step = 0;
    ...
    if (thread_id == 0)
    {
        write_output_file(fmandelbrot_out, mandelbrot,
                          mandelbrot_args->width, mandelbrot_args->height);
