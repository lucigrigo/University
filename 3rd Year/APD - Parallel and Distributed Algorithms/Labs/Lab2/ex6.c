...
int main(int argc, char *argv[]) {
  ...
  void *status;
  int i;
  int arguments[NO_THREADS];

  // paralelizare operatii
  r = pthread_create(&threads[0], NULL, fM1, &arguments[0]);
  if (r != 0)
    exit(-1);

  r = pthread_create(&threads[1], NULL, fM2, &arguments[1]);
  if (r != 0)
    exit(-1);

  r = pthread_create(&threads[2], NULL, fM3, &arguments[2]);
  if (r != 0)
    exit(-1);

  r = pthread_create(&threads[3], NULL, fM4, &arguments[3]);
  if (r != 0)
    exit(-1);

  r = pthread_create(&threads[4], NULL, fM5, &arguments[4]);
  if (r != 0)
    exit(-1);

  r = pthread_create(&threads[5], NULL, fM6, &arguments[5]);
  if (r != 0)
    exit(-1);

  r = pthread_create(&threads[6], NULL, fM7, &arguments[6]);
  if (r != 0)
    exit(-1);
  ...
  // paralelizare operatii
  r = pthread_create(&threads2[0], NULL, fC11, &arguments2[0]);
  if (r != 0)
    exit(-1);

  r = pthread_create(&threads2[1], NULL, fC12, &arguments2[1]);
  if (r != 0)
    exit(-1);

  r = pthread_create(&threads2[2], NULL, fC21, &arguments2[2]);
  if (r != 0)
    exit(-1);

  r = pthread_create(&threads2[3], NULL, fC22, &arguments2[3]);
  if (r != 0)
    exit(-1);

  for (i = 0; i < 4; i++) {
    r = pthread_join(threads2[i], &status);
