...
instance Ord Level
    where compare = undefined

instance Eq Level 
    where (==) = undefined

data Level = Level Int (A.Array (Int, Int) Cell) Int Int
    -- deriving (Eq, Ord)
...
genMoves level emptyCells = foldl gen [] emptyCells
    where 
        (Level _ _ dimX dimY) = level
        gen list (Cell (cx, cy) _) = list
             ++ (if (cx > 0) then [(((cx - 1, cy), South), (moveCell (cx - 1, cy) South level))] else [])
             ++ (if (cx < dimX) then [(((cx + 1, cy), North), (moveCell (cx + 1, cy) North level))] else [])
             ++ (if (cy < dimY) then [(((cx, cy + 1), West), (moveCell (cx, cy + 1) West level))] else [])
             ++ (if (cy > 0) then [(((cx, cy - 1), East), (moveCell (cx, cy - 1) East level))] else [])
