...
FootballClub *initialize_clubs(int clubs_no, char **names) {
  FootballClub *list = (FootballClub *)malloc(sizeof(FootballClub));
  if (list == NULL)
    exit(1);
  ...
  while (i < clubs_no) {
    club->name = (char *)malloc(sizeof(names[i]));
    if (club->name == NULL)
      exit(1);
    strcpy(club->name, names[i]);
    club->players = NULL;
...
void sortPlayers(FootballClub **clubsList, char *clubName) {
  ...
  if ((clubPtr->players == NULL) ||
      ((clubPtr->players->next == NULL) && (clubPtr->players->prev == NULL))) {
    return;
  }
...
void addToInjuredPlayers(FootballClub *clubsList, char *clubName,
                         Player *playerToAdd) {
  FootballClub *clubPtr = clubsList;
  // cautarea clubului
...
void destroy_club_list(FootballClub *clubs) {
  ...
  FootballClub *nextClub = clubs->next;
  free(clubs->name);
  free(clubs);
  destroy_club_list(nextClub);
}
