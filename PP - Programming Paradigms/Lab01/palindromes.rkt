...
(define (num->base n b)
  (cond
    ((equal? n 0) `())
    (else (append (num->base(quotient n b) b) (list (modulo n b))))))
...
(define palindrome-list (list 2 3 4 5 6 7 8 9 10))

(define (all-palindromes-count n Bases)
  (if (null? Bases)
      0
      (if (palindrome? (num->base n (car Bases)))
             (+ 1 (all-palindromes-count n (cdr Bases)))
           (all-palindromes-count n (cdr Bases)))))

(define (first-b-pal start b)
  (if (>= (all-palindromes-count start palindrome-list) b)
      start
